    ...
    local function updateButtonText()
        if multi then
            if #selected == 0 then
                ...
                if #selected <= 3 then
                    valueLabel.Text = table.concat(selected, ", ") .. " โ–ผ"
                else
                    local shown = {selected[1], selected[2], selected[3]}
                    valueLabel.Text = table.concat(shown, ", ") .. " ... ("..#selected..") โ–ผ"
                end
            end
        else
            if #selected == 0 then
                ...
                valueLabel.Text = selected[1] .. " โ–ผ"
            end
        end
    end
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if opened then
            if not searchBox then
                ...
                searchBox.Text = ""
                searchBox.PlaceholderText = "เธเนเธเธซเธฒ..."
